$(document).ready ->
  ...
  if $("#stats-chart2").length != 0
    ...
    $("#stats-chart2").bind "plotclick", (event, pos, item) ->
      if item
        alert "Yeah! You just clicked on point " + item.dataIndex + " in " + item.series.label + "."
  # --------------------------------------------------------------------------------------------------------------------
  ...
  if $("#stats-chart3").length != 0
    ...
    getRandomData = ->
      datareal = datareal.slice(1)  if datareal.length > 0
      while datareal.length < totalPoints
        prev = (if datareal.length > 0 then datareal[datareal.length - 1] else 50)
    ...
    $(this).bind "plothover", (event, pos, item) ->
      if item
        if (previousLabel isnt item.series.label) or (previousPoint isnt item.dataIndex)
          ...
          showTooltip item.pageX, item.pageY, color, "<strong>" + item.series.label + "</strong><br>" + item.series.xaxis.ticks[x].label + " : <strong>" + y + "</strong> °C"
      else
        $("#tooltip").remove()
        previousPoint = null
# --------------------------------------------------------------------------------------------------------------------